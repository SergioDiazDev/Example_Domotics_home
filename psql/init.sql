...
('Salón'), ('Cocina'), ('Dormitorio Principal'), ('Dormitorio Secundario'), ('Baño'), ('Despacho'), ('Comedor'), ('Terraza'), ('Garaje'), ('Sótano');


-- Inserción de datos en la tabla Sensors
INSERT INTO Sensors (type, room_id)
SELECT 'Temperature', id FROM Rooms WHERE name IN ('Salón', 'Cocina', 'Dormitorio Principal', 'Baño', 'Despacho', 'Garaje', 'Sótano');

INSERT INTO Sensors (type, room_id)
SELECT 'Humidity', id FROM Rooms WHERE name IN ('Salón', 'Cocina', 'Baño', 'Despacho', 'Garaje', 'Sótano');

INSERT INTO Sensors (type, room_id)
SELECT 'Light', id FROM Rooms WHERE name IN ('Salón', 'Cocina', 'Dormitorio Principal', 'Dormitorio Secundario', 'Despacho', 'Garaje', 'Sótano');
...
-- Inserción de datos en la tabla Sensor_Readings
INSERT INTO Sensor_Readings (sensor_type, room_id, value, timestamp)
SELECT 'Temperature', id, random() * (30 - 15) + 15, NOW() - INTERVAL '1 day' * i
FROM Rooms, generate_series(0, 240, 1) AS i
WHERE name IN ('Salón', 'Cocina', 'Dormitorio Principal', 'Baño', 'Despacho','Garaje','Sótano');

INSERT INTO Sensor_Readings (sensor_type, room_id, value, timestamp)
SELECT 'Humidity', id, random() * (70 - 30) + 30, NOW() - INTERVAL '1 day' * i
FROM Rooms, generate_series(0, 240, 1) AS i
WHERE name IN ('Salón', 'Cocina', 'Baño', 'Despacho','Garaje','Sótano');

INSERT INTO Sensor_Readings (sensor_type, room_id, value, timestamp)
SELECT 'Light', id, random() * (1000-0) + 0, NOW() - INTERVAL '1 day' * i
FROM Rooms, generate_series(0, 240, 1) AS i
WHERE name IN ('Salón', 'Cocina', 'Dormitorio Principal', 'Dormitorio Secundario', 'Despacho', 'Garaje', 'Sótano');
